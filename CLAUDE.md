...
**Two advancement systems, doing different jobs.** `advancement/recipes/` holds 58
recipe-book unlocks keyed to each recipe's least common ingredient (machine recipes
excluded — custom recipe types never appear in the book). `advancement/*.json` is the
visible 26-node story tab, generated by `tools/gen_advancements.py`: one root off a
rough stone, then the metal branch (hammer -> wire -> band, crucible -> three alloys)
and the stone branch (tumbler -> grit -> tumbled -> trinket, or saw -> slab -> wheel ->
either rouge/cabochon or faceting/faceted). Diamond ring is a goal, star garnet the
challenge capstone. **Criteria are `inventory_changed` on items or stage tags almost
everywhere** — block/location predicates are the ones that silently stop firing; the
two exceptions (`enchanted_item` for Brilliance, `villager_trade` pinned to
jeweler-only stock because vanilla has no profession predicate) carry comments in the
generator. Nine stage tags (`#carvercraft:rough_stones`, `tumbled_stones`, `slabs`,
`cabochons`, `faceted_gems`, `wires`, `bands`, `trinkets`, `cabochon_rings`) fall out
of it and are generally useful. The validator enforces both systems: recipe coverage
both directions, and for the story tab that parents resolve, titles/descriptions have
lang entries, icons are real items, and exactly one root has a background.
